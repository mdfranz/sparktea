...
func parseCLIFlags(args []string) (cliOptions, error) {
	...
	fs.StringVar(&opts.model, "model", "", "Model ID to use with -prompt, e.g. claude-haiku-4-5-20251001 (see -list-models). Defaults to the first available model.")
	...
	fs.BoolVar(&opts.listModels, "list-models", false, "List available model labels (API key present) and exit.")
...
// resolveModel finds the catalog entry matching id: an exact (case
// insensitive) match against modelID or label first — modelID is the
// expected normal case for scripting (no spaces/quoting, e.g.
// claude-haiku-4-5-20251001), label an alternative for the display name —
// then a unique substring match against either. An empty id picks the first
// available option, matching the interactive picker's default.
func resolveModel(options []modelOption, id string) (modelOption, error) {
	if id == "" {
		if len(options) == 0 {
	...
	case 0:
		return modelOption{}, fmt.Errorf("no available model matches %q (see -list-models)", id)
	default:
		ids := make([]string, len(matches))
		for i, m := range matches {
			ids[i] = m.modelID
		}
		return modelOption{}, fmt.Errorf("%q matches multiple models: %s", id, strings.Join(ids, ", "))
	}
}
